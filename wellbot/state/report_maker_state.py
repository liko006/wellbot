...
class ReportMakerState(rx.State):
    ...
    def _classify_intent(self, user_input: str, has_outline: bool) -> str:
        prompt = (
            ...
            f"[사용자 입력]\n{user_input}\n\n"
            "분류 기준:\n"
            "- outline : 보고할 주제/내용(사안·배경·경과·계획 등)이 제시됨. '설명해줘/정리해줘'가 "
            "붙어 있어도 새 주제·내용이 함께 오면 outline. 아웃라인 없음 상태에서 실질 내용이 들어오면 거의 항상 outline.\n"
            "- edit    : 아웃라인 보유 중 + 수정 요청(수정·바꿔·추가·삭제·줄여·늘려) 또는 수치·일정·근거 등 새 값 보완.\n"
            "- chat    : (a) 인사말·잡담·감탄·10자 미만의 의미 없는 입력(예: '안녕', '고마워', 'ㅇㅇ') — "
            "아웃라인 유무와 무관하게 chat. (b) 아웃라인 보유 중 + 내용을 바꾸지 않고 묻기만 하는 후속 질문"
            "(예: '이 수치 어디서 나왔어', '왜 그렇게 했어', '무슨 뜻이야').\n"
            "판단 우선순위: 보고할 실질 주제·내용이 있으면 outline > 인사·잡담·무의미 입력이면 chat.\n"
