...
app.add_page(ai_services_page, route="/ai-services", title="WellBot - AI 업무 특화 서비스", on_load=AuthState.check_auth)
app.add_page(report_checker_page, route="/ai-services/report-checker", title="WellBot - 보고서 오류 탐지", on_load=AuthState.check_auth)
app.add_page(report_maker_page, route="/ai-services/report-generator", title="WellBot - 보고서 draft 지원", on_load=[AuthState.check_auth, ReportMakerState.on_load])
app.add_page(report_maker_style_page, route="/ai-services/report-generator/style", title="WellBot - 작성 가이드 편집", on_load=[AuthState.check_auth, ReportMakerState.load_style_editor])
